...
func TestDuplicateRuleNameInSameRoleRejected(t *testing.T) {
	src := `pack "app.security.access" version 1
type = "access_policy"
...
`
	...
	if _, err := access.Decode(f); err == nil {
		t.Fatal("expected duplicate rule name in same role to fail")
	}
}

func TestSameRuleNameInDifferentRolesAllowed(t *testing.T) {
	src := `pack "app.security.access" version 1
type = "access_policy"
...
`
	...
	if _, err := access.Decode(f); err != nil {
		t.Fatalf("same rule name in different roles should be allowed: %v", err)
	}
}

func TestAutoGeneratedRuleNameIncludesRole(t *testing.T) {
	src := `pack "app.security.access" version 1
type = "access_policy"
  ...
  role "admin":
    allow:
      read:
        - "*"
`
	f, err := parser.ParseString("rolename.elu", src)
	if err != nil {
		t.Fatal(err)
	}
	p, err := access.Decode(f)
	...
	if !found {
		t.Fatal("expected auto-generated rule name to start with role prefix")
	}
}
